...
router.put("/jobs/:id", jwtAuth, (req, res) => {
  ...
  Job.findOne({
    ...
    .then((job) => {
      ...
      if (data.employerDescription) {
        job.employerDescription = data.employerDescription;
      }
      job
        .save()
        .then(() => {
...
router.delete("/applications/:id", jwtAuth, (req, res) => {
  const user = req.user;
  if (user.type != "applicant") {
    res.status(401).json({
      message: "You don't have permissions to delete the application",
    });
    return;
  }
        ...
        .then((job) => {
          ...
          Application.countDocuments({
            ...
            .then((activeApplicationCount) => {
              if (activeApplicationCount < 1000000000) {
                Application.countDocuments({
                  userId: user._id,
                  status: {
                    $nin: ["rejected", "deleted", "cancelled", "finished"],
